...
const validateRegisterData = [
    ...
    body('password').trim().notEmpty().withMessage('Mật khẩu không được bỏ trống')
        .isLength({ min: 8 }).withMessage('Mật khẩu phải có độ dài từ 8 ký tự trở lên')
        .matches(/^(?=.*[A-Z])(?=.*[0-9])(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/)
        .withMessage('Mật khẩu phải chứa ít nhật 1 chữ hoa, 1 ký tự đặc biệt và 1 chữ số'),
    // password confirm can not be empty
    body('passwordConfirm').trim().notEmpty().withMessage('Vui lòng xác nhận mật khẩu'),
